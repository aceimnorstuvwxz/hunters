...
USING_NS_CC;
int HealthBeat::_scheduleCnt = 0;

void HealthBeat::init(BeatFieldProtocal* beatFieldProtocal, cocos2d::Layer* mainLayer, cocos2d::Camera* mainCamera, const BeatFieldLineType& line, float speed)
{
    ...
    _pxContent->configSopx("sopx/beats/health_beat_content.png.sopx");
...
BeatTouchResult HealthBeat::op_touchBegin(const BeatFieldLineType& line)
{
...
BeatTouchResult HealthBeat::op_touchEnd(const BeatFieldLineType& line)
{
    // 并不是长按beat
    return {false, false, false, false};
}

bool HealthBeat::op_update(float dt)
{
    // 移动
...
void HealthBeat::clear() //外部要求它从场景中删除
{
    // beat执行撞击后消散的动画
