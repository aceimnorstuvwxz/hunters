...
void SimplePixelNode::config(const std::vector<PixelUnit>& data)
{
    ...
    for (auto pix : data) {
        ...
        genface(0,1,2,3, color4b2color3b(pix.color), relativePos);//正
        genface(5,4,7,6, color4b2color3b(pix.color), relativePos);//背

        if (pix.R) genface(4,0,3,7, color4b2color3b(pix.color), relativePos);
        if (pix.L) genface(1,5,6,2, color4b2color3b(pix.color), relativePos);
        if (pix.T) genface(4,5,1,0, color4b2color3b(pix.color), relativePos);
        if (pix.B) genface(6,7,3,2, color4b2color3b(pix.color), relativePos);
    }
